...
使用您刚刚定义的路径实例化 `Redactor` 类。`try-finally` 模式可确保资源得到正确释放。
...
在本例中，我们移除了所有注解。您可以将 `DeleteAnnotationRedaction` 替换为任何其他编辑类型（例如，`DeleteTextRedaction`、`RedactImageRedaction`）。
...
按照这四个步骤操作，您就成功编写了**Java 代码编辑方法**，该代码可以加载本地文档、应用编辑并保存编辑后的文件。
