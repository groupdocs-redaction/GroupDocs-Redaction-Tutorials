...
**Q: 我可以编辑受密码保护的文档中的注释吗？**  
A: 可以。在创建 `Redactor` 实例之前，使用相应的密码加载文档。

**Q: GroupDocs.Redaction 是否支持其他注释类型（例如高亮、形状）？**  
A: 该库主要针对基于文本的注释。对于图形元素，建议先将页面光栅化。

**Q: 我如何一次应用多个编辑规则？**  
A: 多次调用 `redactor.apply()`，每次使用不同的 `AnnotationRedaction` 或其他编辑对象。

**Q: 是否可以在保存前预览编辑效果？**  
A: 在应用编辑后，您可以将文档导出为 PDF 并手动检查。
