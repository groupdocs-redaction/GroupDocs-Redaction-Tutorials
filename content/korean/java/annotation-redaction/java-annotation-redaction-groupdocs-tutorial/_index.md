...
## 자주 묻는 질문
**Q: 암호로 보호된 문서의 주석을 Redact할 수 있나요?**  
A: 예. `Redactor` 인스턴스를 만들기 전에 적절한 비밀번호로 문서를 로드하십시오.

**Q: GroupDocs.Redaction이 다른 주석 유형(예: 강조, 도형)을 지원하나요?**  
A: 이 라이브러리는 텍스트 기반 주석에 중점을 둡니다. 그래픽 요소의 경우 먼저 페이지를 래스터화하는 것을 고려하십시오.

**Q: 여러 Redact 규칙을 한 번에 적용하려면 어떻게 해야 하나요?**  
A: 서로 다른 `AnnotationRedaction` 또는 기타 Redact 객체를 사용하여 `redactor.apply()`를 여러 번 호출하면 됩니다.

**Q: 저장하기 전에 Redact를 미리 볼 수 있나요?**  
A: Redact를 적용한 후 문서를 PDF로 내보내고 수동으로 확인할 수 있습니다.
