...
**س: هل يمكنني حذف التعليقات التوضيحية في المستندات المحمية بكلمة مرور؟**  
ج: نعم. حمِّل المستند باستخدام كلمة المرور المناسبة قبل إنشاء كائن `Redactor`.

**س: هل يدعم GroupDocs.Redaction أنواعًا أخرى من التعليقات (مثل التظليل أو الأشكال)؟**  
ج: تركز المكتبة على التعليقات النصية. بالنسبة للعناصر الرسومية، يُنصح بتحويل الصفحة إلى صورة أولًا.

**س: كيف يمكنني تطبيق قواعد حذف متعددة في آن واحد؟**  
ج: استدعِ `redactor.apply()` عدة مرات، كل مرة مع `AnnotationRedaction` أو كائن حذف آخر مختلف.

**س: هل يمكن معاينة الحذف قبل الحفظ؟**  
ج: يمكنك تصدير المستند إلى PDF بعد تطبيق الحذف ومراجعته يدويًا.
