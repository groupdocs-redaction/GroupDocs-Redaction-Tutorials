...
**Q: 能否在受密碼保護的文件中進行註釋遮蔽？**  
A: 能。先以正確的密碼載入文件，再建立 `Redactor` 實例。

**Q: GroupDocs.Redaction 是否支援其他註釋類型（例如高亮、圖形）？**  
A: 此函式庫主要針對文字型註釋。若需處理圖形元素，建議先將頁面光柵化。

**Q: 如何一次套用多條遮蔽規則？**  
A: 可多次呼叫 `redactor.apply()`，每次傳入不同的 `AnnotationRedaction` 或其他遮蔽物件。

**Q: 能否在儲存前預覽遮蔽結果？**  
A: 可以在套用遮蔽後將文件匯出為 PDF，手動檢查。
